import { assets, infoList, toolsData } from '@/assets/assets'
import Image from 'next/image'
import React from 'react'

const About = () => {
    return (
        <div id='about' className='w-full px-[12%] py-5 scroll-mt-20'>
            <h4 className='text-center mb-2 text-lg font-Ovo'>Introduction</h4>
            <h2 className='text-center text-5xl font-Ovo'>About Me</h2>

            <div className='flex w-full flex-col lg:flex-row items-center gap-20 my-20'>
                <div className='w-64 sm:w-80 rounded-3xl max-w-none'>
                    <Image className='w-full rounded-3xl' src={assets.profile_img} alt='user' />
                </div>
                <div className='flex-1'>
                    <p style={{ fontSize: "18px" }} className='mb-10 max-w-2xl font-Ovo'>"I am currently pursuing B.Tech in Computer Science from AKTU. I am passionate about technology and constantly working to enhance my skills in programming, software development, and problem-solving."</p>

                    <div className='grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl'>
                        <ul className='border-[0.5px] border-gray-400 rounded-xl p-6 cursor-pointer hover:-translate-y-1  duration-500'>
                            <Image className='w-7 mt-3 mb-3' src={assets.code_icon} alt='img'/>
                            <h3 className='my-4 font-semibold text-gray-700'>Languages</h3>
                         <p className='text-gray-600 text-sm'>HTML,CSS,JavaScript,<br />ReactJs,NextJs,TypeScript</p>
                        </ul>
                        <ul className='border-[0.5px] border-gray-400 rounded-xl p-6 cursor-pointer  hover:-translate-y-1 duration-500'>
                            <Image className='w-7 mt-3 mb-3' src={assets.edu_icon} alt='img' />
                            <h3 className='my-4 font-semibold text-gray-700'>Education</h3>
                            <p className='text-gray-600 text-sm'>B.Tech in Computer Science</p>
                        </ul>
                    </div>


                    <h4 className='my-6 text-gray-700'>Tools I Use</h4>

                    <ul className='flex items-center gap-3 sm:gap-5' >
                        {toolsData.map((tool, index) => (
                            <li className='flex items-center justify-center w-12 sm:w-14 aspect-square border border-gray-400 rounded-lg cursor-pointer hover:-translate-y-1 duration-500 ' key={index}>
                                <Image src={tool} alt='Tool' className='w-5 sm:w-7' />
                            </li>
                        ))}
                    </ul>
                </div>
            </div>
        </div>
    )
}

export default About
